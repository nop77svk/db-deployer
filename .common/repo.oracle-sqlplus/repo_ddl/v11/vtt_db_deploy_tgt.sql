...
with xyz as (
    select
        regexp_substr(txt_left_side, '^dpltgt_(.*)_(user|password|db|tbs_.*|tech)$', 1, 1, 'i', 1) as deploy_target,
        regexp_substr(txt_left_side, '^dpltgt_(.*)_(user|password|db|tbs_.*|tech)$', 1, 1, 'i', 2) as deploy_var_name,
        txt_right_side as deploy_var_value
    from vtt_db_deploy_tgt X
    where txt_left_side like 'dpltgt!_%' escape '!'
)
select deploy_target as target_name, user_val as target_user, password_val as target_password, db_val as target_db
from xyz pivot (
